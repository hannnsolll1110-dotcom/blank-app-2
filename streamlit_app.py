...
def load_data():
    ...
    # [2] 보기 싫은 결측치(None, NaN) 처리 ✨
    # 전화번호가 비어있으면 '-'로 채우기
    df['업소 전화번호'] = df['업소 전화번호'].fillna("-") 
    
    # [3] 가나다순 정렬 ✨
    # 업소명을 기준으로 오름차순(가나다순) 정렬
    df = df.sort_values(by='업소명', ascending=True)
