...
import plotly.express as px  # [추가] 시각화를 위한 라이브러리
...
# -----------------------------------------------------------------------------
# 3. 사이드바 (메뉴 선택) - 여기서 화면이 갈립니다!
# -----------------------------------------------------------------------------
st.sidebar.header("목록 이동")
menu = st.sidebar.radio("메뉴를 선택하세요", ["🔍 가게 찾기 & 시민 제보", "📊 데이터 분석 시각화"])


# =============================================================================
# [화면 1] 가게 찾기 & 시민 제보 (기존 코드 영역)
# =============================================================================
if menu == "🔍 가게 찾기 & 시민 제보":
...
# =============================================================================
# [화면 2] 데이터 분석 시각화 (새로 추가된 부분)
# =============================================================================
elif menu == "📊 데이터 분석 시각화":
    st.title("📊 서울시 착한가격업소 데이터 분석")
    st.markdown("서울시 데이터를 한눈에 파악해보세요!")
    st.divider()

    # ---------------------------------------------------------
    # [시각화 1] 자치구별 착한가격업소 수 (막대 그래프)
    # ---------------------------------------------------------
    st.subheader("🏙️ 어느 구에 착한 업소가 가장 많을까요?")

    # 1. 데이터 가공 (구별 개수 세기)
    gu_counts = df['자치구'].value_counts().reset_index()
    gu_counts.columns = ['자치구', '업소수'] # 컬럼명 변경
    ...
    fig1 = px.bar(
        ...
        x='자치구', 
        y='업소수', 
        text='업소수', # 막대 위에 숫자 표시
        color='업소수', # 개수에 따라 색상 다르게
        color_continuous_scale='Blues', # 파란색 계열
        title="서울시 자치구별 착한가격업소 분포"
    )

    # 3. 그래프 스타일 다듬기
    fig1.update_traces(textposition='outside') # 숫자를 막대 바깥에 표시
    fig1.update_layout(xaxis_title="자치구", yaxis_title="업소 수 (곳)")
    ...
    top_count = gu_counts.iloc[0]['업소수']
    
    st.info(f"💡 분석 결과, **{top_gu}**가 총 **{top_count}곳**으로 착한가격업소가 가장 많이 지정되어 있습니다.")
