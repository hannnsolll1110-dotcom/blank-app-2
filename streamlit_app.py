...
# -----------------------------------------------------------------------------
# [중요] 시각화를 위한 라이브러리 임포트 (이게 꼭 있어야 해요!)
# -----------------------------------------------------------------------------
import altair as alt  # 막대그래프, 히트맵용 (스트림릿 내장)
import plotly.express as px  # 도넛차트용 (예쁜 그래프)
...
# [PAGE 2] 데이터 분석 시각화
    ...
    tab1, tab2, tab3 = st.tabs(["🏆 자치구별 분포", "🍕 업종별 비율", "🔥 지역x업종 히트맵"])

    # [Tab 1] 자치구별 순위 (Bar Chart)
    with tab1:
        st.markdown(f"#### 🏢 {selected_gu if selected_gu != '전체' else '서울시'} 착한업소 분포")
        st.caption("어느 지역에 착한가격업소 인프라가 집중되어 있는지 정량적으로 비교합니다.") # 설명 추가
    ...
    # [Tab 2] 업종별 비율 (Donut Chart) - Plotly 사용!
    with tab2:
        st.markdown("#### 🍴 업종별 점유율")
        st.caption("물가 안정 정책이 주로 어떤 업종(외식업 등)에 집중되어 있는지 비율을 확인합니다.") # 설명 추가
        ...
        if cat_counts.empty:
            ...
            # px를 사용하려면 import plotly.express as px가 필수!
